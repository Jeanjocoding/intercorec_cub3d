...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/06/09 11:50:56 by tlucille          #+#    #+#             */
/*   Updated: 2020/06/09 11:50:58 by tlucille         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_right_char_mid(char c, int x, int y, t_fov *fov)
{
	...
	else if (c == 'N' || c == 'S' || c == 'E' || c == 'W')
	{
		...
		{
			free_int_tab(&g_worldmap, g_mlen);
			error_quit("Error\ntwo different initial positions", NULL);
		}
		g_worldmap[x][y] = 0;
	}
	else if (c != ' ')
	{
		free_int_tab(&g_worldmap, g_mlen);
		error_quit("Error\ninvalid map, uknown object number", NULL);
	}
	return (0);
}
